...
Avatar::Avatar(QWidget* parent, LabelPosition pos, const QImage& img, const QString& name,
  ...
  : QWidget(parent), totalTime(totalTime), currentTime(singleTime), singleTime(singleTime) {
    // init form
    ...
    int avatarHeight = 60;
//        lblName->geometry().height() + lblLeftTime->geometry().height() + layout->verticalSpacing();
    ...
    connect(&timer, &QTimer::timeout, this, [=] {
        ...
        if (this->currentTime <= 50) {
            this->currentTime = singleTime;
        }
        QTime newTime = this->totalTime.addMSecs(-interval);
        lblLeftTime->setText(newTime.toString("mm:ss"));
        this->totalTime.setHMS(newTime.hour(), newTime.minute(), newTime.second(), newTime.msec());
        if (this->totalTime.isNull()) {
            timer.stop();
            emit timeout();
        }
