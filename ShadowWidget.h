...
public:
    explicit ShadowWidget(QWidget* parent = nullptr)
        : Base(parent), _title(new TitleBar(this)), _layout(new QVBoxLayout()), _ptr(this) {
...
protected:
    void paintEvent(QPaintEvent* event) override {
        ...
        if (_ptr->windowState() == Qt::WindowMaximized
            || _ptr->windowState() == Qt::WindowFullScreen) {
            _rect.setRect(0, 0, _ptr->width(), _ptr->height());
        }
        else
            _rect.setRect(_shadowWidth, _shadowWidth, _ptr->width() - 2 * _shadowWidth,
                          _ptr->height() - 2 * _shadowWidth);
        if (_borderRadius == 0) {
        ...
        if (_ptr->windowState() != Qt::WindowMaximized
            ...
            for (int i = 1; i < _shadowWidth; i++) {
                ...
                if (_borderRadius)
                    painter.drawRoundedRect(_rect.x() - i, _rect.y() - i, _rect.width() + 2 * i,
                                            _rect.height() + 2 * i, _borderRadius, _borderRadius);
                else
                    painter.drawRect(_rect.x() - i, _rect.y() - i, _rect.width() + 2 * i,
                                     _rect.height() + 2 * i);
            }
        }
        ...
        case QEvent::MouseMove:
            if (_pressed) {
                if (_resizable && _cursorShapeChanged) {
                    ...
                    if (newRect.isValid()) {
                        if (_ptr->minimumWidth() < newRect.width()
                            && _ptr->minimumHeight() < newRect.height())
    ...
    void initShadowWidget(QLayout* content) {
        if (auto* p = qobject_cast<QLayout*>(content->parent()); p)
            p->removeItem(content);
        if constexpr (std::is_same_v<Base, QMainWindow>) {
            ...
            _layout->addLayout(content);
        _rect = QRect(_shadowWidth, _shadowWidth, _ptr->width() - 2 * _shadowWidth,
                      _ptr->height() - 2 * _shadowWidth);
    }
...
private:
    ...
    void recalculate(const QPoint& globalMouse) {
        ...
        setFlag(Right, globalMouseX >= frameX + frameRect.width() - _shadowWidth
                           && globalMouseX <= frameX + frameRect.width());
        setFlag(Top, globalMouseY >= frameY && globalMouseY <= frameY + _shadowWidth);
        setFlag(Bottom, globalMouseY >= frameY + frameRect.height() - _shadowWidth
                            && globalMouseY <= frameY + frameRect.height());
    }

    inline void setFlag(Edge flag, bool value) {
        _mouseEdgeFlags = static_cast<Edge>(value ? int(_mouseEdgeFlags) | int(flag)
                                                  : int(_mouseEdgeFlags) & ~int(flag));
    }
    inline bool testFlag(Edge flag) const noexcept {
        return (_mouseEdgeFlags & static_cast<int>(flag)) == static_cast<int>(flag)
               && (static_cast<int>(flag) != 0 || _mouseEdgeFlags == static_cast<int>(flag));
    }
